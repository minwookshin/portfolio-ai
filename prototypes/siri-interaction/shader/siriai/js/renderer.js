...
import { VERTEX_SHADER } from './shaders/vertex.glsl.js';
import { WAVE_FRAGMENT_SHADER } from './shaders/wave.frag.glsl.js';
import { DOTS_FRAGMENT_SHADER } from './shaders/dots.frag.glsl.js';
import { BACKGROUND_FRAGMENT_SHADER } from './shaders/background.frag.glsl.js';
import { EFFECT_COMPOSITE_FRAGMENT_SHADER } from './shaders/effect-composite.frag.glsl.js';
import { GLASS_COMPOSITE_FRAGMENT_SHADER } from './shaders/glass-composite.frag.glsl.js';
import { WAVE_PRESETS, waveUniforms, dotsUniforms } from './shaders/uniforms.js';
...
// pill) the corners stop growing so the glass stays a rounded square.
const CORNER_RADIUS_MAX_PX = 75;
...
export class SiriRenderer {
	...
	// pill: width/height lerp toward sizes.answer, cornerRadius stays at
	// min(half) so the shape remains a capsule the whole way — capped at the
	// resting pill's capsule radius, so a tall answer becomes a rounded
	// square instead of a circle (square text must stay inside the glass).
	_layout(surface, sizes) {
		...
		return {
			...
			margin,
			cornerRadius: Math.min(Math.min(coreWidth, coreHeight) * 0.5, CORNER_RADIUS_MAX_PX * this.dpr),
			containerStrength:
