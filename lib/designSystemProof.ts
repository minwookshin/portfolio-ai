...
export const DESIGN_SYSTEM_TOKENS = {
  ...
  signals: [
    ...
    { role: "dot.work", cssVariable: "--signal-dot-size-work", value: "7px", usage: "slightly stronger selected-work homepage rows" },
    { role: "dot.hollow", cssVariable: "--signal-dot-size-hollow", value: "8px", usage: "archive and detail section hollow dots" },
    { role: "dot.homeSection", cssVariable: "--signal-dot-size-section", value: "9px", usage: "homepage expandable section hollow dots" },
    ...
    { role: "motion.nudge", cssVariable: "--signal-nudge", value: "2px", usage: "lateral movement for signal, label, and action-chip intent" },
    { role: "motion.draw", cssVariable: "--signal-draw-duration", value: "140ms", usage: "stroke draw duration for Material arrow paths" },
  ],
  elevation: [
...
export function generateDesignSystemMarkdown() {
  return lines([
    ...
    "",
    "### Outline signals",
    ...DESIGN_SYSTEM_TOKENS.signals.map((token) => `- ${token.role}: ${token.value} (${token.cssVariable}) - ${token.usage}`),
    "",
    "### Motion",
